...
| `shareDirectoryDownloadLimiter` | 1 min | 5 | IP | `GET /s/:token/download` — same zip-streaming concern as `directoryDownloadLimiter`, tighter still since it's on the fully public surface with no user id to key on |
