...
Enforced on: `/directory/*`, `/file/*`, `/subscriptions/*` (all mutating methods, applied at the
router-group level in `src/routes/index.js`), plus `/auth/logout` and `/auth/logout-all`
specifically (the only `/auth/*` routes that require an existing session).
